...
conn_handler(uint8_t *packet, conn_param *cparam, size_t max)
{
	...
	if (((cparam->con_flag & 0x01) != 0) ||
		...
		log_info("Malformed CONNECT: conn flag conflicts with will msg");
		return PROTOCOL_ERROR;
	}
	...
	if (len_of_str == 0) {
		...
		log_info("Malformed CONNECT: incomplete utf8 in clientid");
		return (PROTOCOL_ERROR);
	} else if (cparam->pro_ver == MQTT_PROTOCOL_VERSION_v31 &&
	    len_of_str > 23) {
		log_info("CLIENT_IDENTIFIER_NOT_VALID: clientid invalid!");
	...
	if (rv == 0 && (cparam->con_flag & 0x80) > 0) {
		...
		rv                   = len_of_str <= 0 ? PAYLOAD_FORMAT_INVALID : 0;
		if (len_of_str == 0) {
			log_warn("0 length of username found! It is a malformed CONNECT!\
					  But we let it bypass rules due to compatiblity.");
			rv = 0;
		} else if (rv != 0) {
			return rv;
		}
	...
	if (rv == 0 && (cparam->con_flag & 0x40) > 0) {
		...
		rv                   = len_of_str <= 0 ? PAYLOAD_FORMAT_INVALID : 0;
		if (len_of_str == 0) {
			log_warn("0 length of passwd found! It is a malformed CONNECT!\
					  But we let it bypass rules due to compatiblity.");
			rv = 0;
		} else if (rv != 0) {
